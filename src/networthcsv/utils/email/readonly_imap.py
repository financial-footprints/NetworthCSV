...
class ReadOnlyImapClient:
    ...
    def connect(
        ...
        *,
        ...
        if typ != "OK":
            ...
            raise StageError(f"IMAP login failed: {detail}")
        return client
    ...
    def _login(self, username: str, password: str) -> tuple[str, list[bytes | None]]:
        self._record("LOGIN")
        return cast(
            tuple[str, list[bytes | None]], self._conn.login(username, password)
        )
    ...
    def search(
        self, charset: str | None, *criteria: str
    ...
    def uid_fetch(
        ...
        if "BODY.PEEK" not in parts.upper() and "RFC822" not in parts.upper():
            raise RuntimeError(
                "read-only IMAP client only allows BODY.PEEK or RFC822 fetch"
            )
